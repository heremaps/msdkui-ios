...
class LocationBasedViewControllerTests: XCTestCase {
    /// The view controller to be tested - it must implement LocationBasedViewController protocol.
    var viewControllerUnderTest: WaypointViewController?

    /// The mock notification center used to verify expectations.
    var mockNotificationCenter = NotificationCenterObservingMock()

    /// The mock URL opener used to verify expectations.
    var mockURLOpener = URLOpenerMock()

    override func setUp() {
        super.setUp()

        // Create the viewControllerUnderTest object
        viewControllerUnderTest = UIStoryboard.instantiateFromMainStoryboard() as WaypointViewController
    ...
    func testCheckLocationAuthorizationStatus() throws {
        ...
        // Send notification
        mockNotificationCenter.lastBlock?(Notification(name: .UIApplicationDidBecomeActive))
    ...
    func testWhenAppSettingsButtonIsTappedForUnauthorizationLocationStatusAlert() throws {
        ...
        XCTAssertEqual(mockURLOpener.lastURL?.absoluteString, UIApplicationOpenSettingsURLString,
