...
final class GuidanceViewControllerTests: XCTestCase {
    ...
    func testCallbacks() throws {
        ...
        // Is the data passed?
        XCTAssertNotNil(viewControllerUnderTest?.maneuverPanel.data, "Maneuver panel has no data!")
        XCTAssertEqual(viewControllerUnderTest?.maneuverPanel.data, data, "Maneuver panel hasn't the data!")

        viewControllerUnderTest?.guidanceManeuverPanelPresenterDidReachDestination(presenter)

        // Are the Info2 labels highlighted?
        viewControllerUnderTest?.maneuverPanel.info2Labels.forEach {
    ...
    func testGuidanceCurrentStreetNameMonitorDidUpdateCurrentStreetNameWithContent() throws {
        ...
        viewControllerUnderTest.guidanceCurrentManeuverMonitor(viewControllerUnderTest.guidanceCurrentStreetNameMonitor,
    ...
    /// Tests configuration of current street label with current street view model when current street name changes to a empty name.
    ...
    func testIdleTimerIsEnabledByDefault() throws {
        let idleTimerDisabler = try require(viewControllerUnderTest?.idleTimerDisabler)
