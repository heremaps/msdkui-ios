...
final class OptionsViewControllerTests: XCTestCase {
    /// The Option View Controller under test.
    var viewControllerUnderTest: OptionsViewController?

    /// The mock delegate used to check expectations.
    var mockDelegate = OptionsDelegateMock() // swiftlint:disable:this weak_delegate

    override func setUp() {
        super.setUp()

        // Initializes the view from the Storyboard
        viewControllerUnderTest = UIStoryboard.instantiateFromMainStoryboard() as OptionsViewController
    ...
    func testTableView() {
        XCTAssertEqual(viewControllerUnderTest?.tableView.tableFooterView?.bounds.height, 0,
                       "It hides unused table view cells")

        XCTAssertEqual(viewControllerUnderTest?.tableView.accessibilityIdentifier, "OptionsViewController.tableView",
    ...
    func testBackButton() {
        ...
        XCTAssertEqual(viewControllerUnderTest?.backButton.accessibilityIdentifier, "OptionsViewController.back",
    ...
    func testSelectTableViewRowForCarAndRow0() throws {
        try selectTableViewCellFor(transportMode: .car, at: IndexPath(row: 0, section: 0)) { presentedViewController in
            XCTAssertTrue(presentedViewController.panel is RouteTypeOptionsPanel,
                          "It presents the Options Panel View Controller with the correct panel")

            XCTAssertTrue((presentedViewController.panel as? RouteTypeOptionsPanel)?.routingMode === viewControllerUnderTest?.routingMode,
                          "It presents the Options Panel View Controller with the correct panel routing mode")

            XCTAssertEqual(presentedViewController.panelTitle, RouteTypeOptionsPanel.name,
                           "It presents the Options Panel View Controller with the correct panel title")

            XCTAssertTrue(presentedViewController.delegate === viewControllerUnderTest,
    ...
    func testSelectTableViewRowForCarAndRow1() throws {
        try selectTableViewCellFor(transportMode: .car, at: IndexPath(row: 1, section: 0)) { presentedViewController in
            XCTAssertTrue(presentedViewController.panel is TrafficOptionsPanel,
                          "It presents the Options Panel View Controller with the correct panel")

            XCTAssertTrue((presentedViewController.panel as? TrafficOptionsPanel)?.dynamicPenalty === viewControllerUnderTest?.dynamicPenalty,
                          "It presents the Options Panel View Controller with the correct panel dynamic penalty")

            XCTAssertEqual(presentedViewController.panelTitle, TrafficOptionsPanel.name,
                           "It presents the Options Panel View Controller with the correct panel title")
    ...
    func testSelectTableViewRowForCarAndRow2() throws {
        try selectTableViewCellFor(transportMode: .car, at: IndexPath(row: 2, section: 0)) { presentedViewController in
            XCTAssertTrue(presentedViewController.panel is RoutingOptionsPanel,
                          "It presents the Options Panel View Controller with the correct panel")

            XCTAssertTrue((presentedViewController.panel as? RoutingOptionsPanel)?.routingMode === viewControllerUnderTest?.routingMode,
                          "It presents the Options Panel View Controller with the correct panel routing mode")

            XCTAssertEqual(presentedViewController.panelTitle, RoutingOptionsPanel.name,
                           "It presents the Options Panel View Controller with the correct panel title")
    ...
    func testSelectTableViewRowForTruckAndRow2() throws {
        try selectTableViewCellFor(transportMode: .truck, at: IndexPath(row: 2, section: 0)) { presentedViewController in
            XCTAssertTrue(presentedViewController.panel is TunnelOptionsPanel,
                          "It presents the Options Panel View Controller with the correct panel")

            XCTAssertTrue((presentedViewController.panel as? TunnelOptionsPanel)?.routingMode === viewControllerUnderTest?.routingMode,
                          "It presents the Options Panel View Controller with the correct panel routing mode")

            XCTAssertEqual(presentedViewController.panelTitle, TunnelOptionsPanel.name,
                           "It presents the Options Panel View Controller with the correct panel title")
    ...
    func testSelectTableViewRowForTruckAndRow3() throws {
        try selectTableViewCellFor(transportMode: .truck, at: IndexPath(row: 3, section: 0)) { presentedViewController in
            XCTAssertTrue(presentedViewController.panel is HazardousMaterialsOptionsPanel,
                          "It presents the Options Panel View Controller with the correct panel")

            XCTAssertTrue((presentedViewController.panel as? HazardousMaterialsOptionsPanel)?.routingMode === viewControllerUnderTest?.routingMode,
                          "It presents the Options Panel View Controller with the correct panel routing mode")

            XCTAssertEqual(presentedViewController.panelTitle, HazardousMaterialsOptionsPanel.name,
                           "It presents the Options Panel View Controller with the correct panel title")
    ...
    func testSelectTableViewRowForTruckAndRow4() throws {
        try selectTableViewCellFor(transportMode: .truck, at: IndexPath(row: 4, section: 0)) { presentedViewController in
            XCTAssertTrue(presentedViewController.panel is TruckOptionsPanel,
                          "It presents the Options Panel View Controller with the correct panel")

            XCTAssertTrue((presentedViewController.panel as? TruckOptionsPanel)?.routingMode === viewControllerUnderTest?.routingMode,
                          "It presents the Options Panel View Controller with the correct panel routing mode")

            XCTAssertEqual(presentedViewController.panelTitle, TruckOptionsPanel.name,
                           "It presents the Options Panel View Controller with the correct panel title")
    ...
    private func verifyTableViewFor(transportMode: NMATransportMode,
        ...
        XCTAssertEqual(viewControllerUnderTest?.tableView(tableView, numberOfRowsInSection: 0), expectedTitles.count,
        ...
        XCTAssertEqual(rowTitles, expectedTitles,
                       "It returns the correct table view cells",
                       file: file,
                       line: line)
    }

    private func selectTableViewCellFor(transportMode: NMATransportMode,
                                        at indexPath: IndexPath,
                                        completion: (_ presentedViewController: OptionPanelViewController) -> Void) throws {
        // Sets the transport mode
        viewControllerUnderTest?.transportMode = transportMode
        ...
        XCTAssertTrue(viewControllerUnderTest?.presentedViewController is OptionPanelViewController,
