...
DEFAULT_IOS_VERSION = ENV['DEFAULT_IOS_VERSION'] || "11.4"
...
namespace :tools do
    ...
    desc "Build Jazzy API Documentation"
...
def xcodeexport(archive_path:, output_directory:, method:, bundle_id:, provisioning_profile_name:)
...
def downloadizumi(url:, output_directory:)
    # Dependencies
    begin
        require 'open-uri'
        require 'fileutils'
    ...
    # Temporary directory
    izumi_temp = "izumi_temp"

    # Downloads from Izumi to the temporary directory
    content = open(url)
    Zip::File.open_buffer(content) do |zip_file|
        zip_file.each do |entry|
            entry_path = File.join(izumi_temp, entry.name)
            entry.extract(entry_path) unless File.exist?(entry_path)
        end
    end

    # Converts to UTF-8 and replaces exiting files
    Dir["#{izumi_temp}/*"].each do |izumi_language_dir|
        dir_name = File.basename(izumi_language_dir)
        destination_dir_name = "#{output_directory}#{dir_name}/"

        if !Dir[destination_dir_name].empty?
            puts "Copying #{izumi_language_dir} to #{destination_dir_name}"

            Dir["#{izumi_language_dir}/Localizable.strings"].each do |strings_file|
