...
@IBDesignable open class IconButton: UIButton {
    ...
    public enum ButtonType: String {
        ...
        /// Returns the unlocalized `accessibilityIdentifier` string.
    ...
    public var type: ButtonType = .add {
        didSet {
            // Reflect the update
            updateImage()
            setAccessibility()
    ...
    @IBInspectable public var typeProxy: String? {
        get {
            return type.rawValue
        }
        set {
            if let value = newValue,
                let type = ButtonType(rawValue: value) {
                self.type = type
            }
        }
    }
    ...
    private func setAccessibility() {
        ...
        imageView?.accessibilityIdentifier = type.accessibilityIdentifier + ".icon"
    }
}
