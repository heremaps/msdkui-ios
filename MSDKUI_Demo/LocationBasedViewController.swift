...
extension LocationBasedViewController where Self: UIViewController {
    func setUpLocationAuthorizationObserver() {
        appBecomeActiveObserver = notificationCenter.addObserver(forName: .UIApplicationDidBecomeActive, object: nil, queue: nil) { [weak self] _ in
    ...
    private func locationAuthorizationDeniedAction() {
        displayNoLocationAlert()
    }

    private func displayNoLocationAlert() {
        ...
        alert.view.accessibilityIdentifier = "LocationBasedViewController.permissionsAlert"
