...
class ViewController: UIViewController {
    ...
    var showMapView = true {
        didSet {
            ...
            // the right button with the "Expand" image and otherwise
            // hide it: as UIBarButtonItem has no isHidden property, we
            // play with its tintColor property and disable it
            if mapView.isHidden == true {
                rightButton.tintColor = UIColor.colorAccentLight
                rightButton.isEnabled = true
                rightButton.image = UIImage(named: "Expand")
    ...
    @IBAction private func onRight(_: UIBarButtonItem) {
        if waypointListStackView.isHidden == true {
            // Show the waypoint list and show the "Collapse" button
            waypointListStackView.isHidden = false
            rightButton.image = UIImage(named: "Collapse")
        } else {
            // Hide the waypoint list and show the "Expand" button
