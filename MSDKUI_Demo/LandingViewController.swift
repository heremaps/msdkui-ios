...
class LandingViewController: UIViewController {
    ...
    @IBAction private func onInfo(_: UIBarButtonItem) {
        let appVersionStrings = getAppVersionStrings()
        let frameworkVersionString = MSDKUI.Version.getString()
        let msg = String(format: "msdkui_app_versions".localized, arguments: [appVersionStrings.version, appVersionStrings.build, frameworkVersionString])
        let alertController = UIAlertController(title: nil, message: msg, preferredStyle: .alert)
        let cancelAction = UIAlertAction(title: "msdkui_app_ok".localized, style: .cancel) { _ in }

        // UIAlertController accessibility support is terrible! Line breaks are not converted into pauses
        // automatically. So, we want to insert pauses after each msg line read, yet there is no official
        // way to access the UIAlertController title & msg views...
        let msgView = alertController.view.subviews[0].subviews[0].subviews[0].subviews[0].subviews[0].subviews[0]
        msgView.accessibilityLabel = msg.replacingOccurrences(of: "\n", with: ", ")
    ...
    func getAppVersionStrings() -> (version: String, build: String) {
        var version = "?"
        var buildId = "?"
        ...
        if let plistUrl = Bundle.main.url(forResource: "Info", withExtension: "plist"),
            ...
            if let result = try? PropertyListSerialization.propertyList(from: data, options: [], format: nil) as? [String: Any] {
                if let result = result {
                    if let versionFromPlist = result["CFBundleShortVersionString"] as? String {
                        version = versionFromPlist
                    }

                    if let buildIdFromPlist = result["CFBundleVersion"] as? String {
                        buildId = buildIdFromPlist
                    }
                }
            }
        }
