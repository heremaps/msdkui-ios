...
class TunnelOptionsPanelTests: XCTestCase, PickerViewDelegate {
    // The panel
    ...
    func optionTests() {
        ...
        XCTAssertEqual(item?.labels.count, TunnelOptionsPanel.options.count, "The panel has not expected number of options!")

        // Change the selected option
        item?.selectedItemIndex = 3
        ...
        XCTAssertEqual(routingMode.tunnelCategory, .d, "Not the expected MARoutingMode.tunnelCategory value!")
    ...
    func onOptionChanged(_ item: OptionItem) {
        ...
        XCTAssertEqual(item.selectedItemIndex, 3, "Not the expected option selected!")
