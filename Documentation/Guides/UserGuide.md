...
<!-- This guide's version is 1.1 and it is based on the HERE Mobile SDK 3.8 and HERE Mobile SDK UI Kit 1.4.0.-->
...
- [Getting Started - A UI Kit Primer](#getting-started-a-ui-kit-primer)
- [Overview of the UI Kit Primer example](#overview-of-the-ui-kit-primer-example)
- [Designing the app flow](#designing-the-app-flow)
- [Adding UI Kit components](#adding-ui-kit-components)
...
The UI Kit builds upon optimized native platform code to fully support Xcode's _Interface Builder_ and Android Studio's _Layout Editor_ resulting in reduced development time and a faster time to market for your apps.
...
With the HERE Mobile SDK UI Kit, realizing complete apps including comprehensive route planning and state-of-the-art guidance becomes a matter of minutes. While hiding the underlying complexity, you still have all the freedom you need to build unique and powerful apps - take a quick tour with our [UI Kit Primer](#getting-started-a-UI Kit-primer) to see a practical example.

Version 1.x of the UI Kit mainly focuses on enabling user experiences related to route planning and guidance. The HERE Mobile SDK UI Kit components are available for iOS and Android, supporting Java and Kotlin on Android, and Swift on iOS.

## Where to start?
- If you haven't done so, please read our [Quick Start](QuickStart.md) guide to see how you can integrate the UI Kit into your own apps.
- Check the _API Reference_ that comes [bundled](../API_Reference/index.html) within the downloaded UI Kit package.
- In the downloaded UI Kit package you can also find:
  - Numerous [examples](../Guides_Examples/) accompanying this user guide.
  - A HERE MSDK UI [demo app](../../Demo/) showcasing most features of the UI Kit in production-ready quality.
- Read the [UI Kit Primer](#getting-started-a-ui-kit-primer) chapter of this user guide.
...
All UI Kit components support initialization from storyboards or by code (programmatically). For most examples, we recommend using storyboards and Auto Layout. As you can use UI Kit components like any other 3rd party custom controls, the general workflow does not contain any specific treatment. If you are new to working with views and layouts under iOS, we recommend to read Apple's [Auto Layout Guide](https://developer.apple.com/library/archive/documentation/UserExperience/Conceptual/AutolayoutPG/index.html#//apple_ref/doc/uid/TP40010853-CH7-SW1).

## Getting Started - A UI Kit Primer
...
>**Note:** For integrating the UI Kit framework, please have a look at our [Quick Start](QuickStart.md) to safely guide you through the first steps.

You can find the complete example code of the _UIKitPrimer_ app in the [examples section](../Guides_Examples/) of the downloaded UI Kit package. We recommend that you follow this tutorial step-by-step. However, if you get stuck or if you just want to inspect the resulting code of this tutorial, please refer to the app as a point of reference.

## Overview of the UI Kit Primer example
The UI Kit Primer example app consists of three screens illustrating some of the main HERE Mobile SDK UI Kit components _in action_:
...
>**Note:** that these steps do not contain any UI Kit specific behavior. As always, there are many alternative approaches to work with _Interface Builder_. The UI Kit's components will smoothly fit into any workflow that you are used to.

## Adding UI Kit components
Now let's add the first UI Kit components to our new project. As outlined before we want to show two UI Kit components on our main `ViewController`:
- `WaypointList`
- `TransportModePanel`

Since all UI Kit components are direct or indirect children of `UIView`, they behave like any other `UIView` component. However, they are custom controls and they are not part of Apple's object library.
...
![Setting Custom Class](./Images/xcode_custom_class.png)

Now you can Control-drag the components to your `ViewController` to create `IBOutlet` references for later use.

Make sure to import the UI Kit by setting `import MSDKUI` at the top of your class.
...
`NMAMapView`'s super-type is `UIView`, but it depends on the way you integrated the HERE Mobile SDK. Please, make sure to _not_ set the _Module_ type in the _Custom Class_ area of the _Identity Inspector_. In opposition, when manually integrating the `UI Kit`, make sure to _always_ set the `Module`.
...
UI Kit provides the `WaypointEntry` class as a wrapper to allow modification of certain attributes, so that you can set - for example - street names for each waypoint instead of showing the raw coordinates. Please see the [Quick Start](QuickStart.md) guide for an example.
```swift
...
Note that order matters, but don't worry, the `WaypointList` UI Kit component by default already provides drag handles to change the order afterwards.
...
To indicate that a waypoint was tapped, we tweak a little bit the default styles that are available for each UI Kit component.
UI Kit provides a `Styles` singleton to allow you to change the default styles at any time to achieve an immediate effect or to change the look-and-feel of a component on-the-fly.
```swift
...
These are not the only customizations you can make to a UI Kit component. For example, the following styles can be set for the `WaypointList` component:
- `waypointListBackgroundColor: UIColor`
- `waypointListAlwaysBounceVertical: Bool`
- `waypointListSeparatorColor: UIColor`
- `waypointListSeparatorStyle: UITableViewCellSeparatorStyle`
- `waypointListSeparatorInset: UIEdgeInsets`
- `waypointListFlashColor: UIColor`
- `waypointListFlashDurationSeconds: Double`
- `waypointItemBackgroundColor: UIColor`
- `waypointItemTextColor: UIColor`
- `waypointItemLabelLeadingInset: CGFloat`
...
As mentioned in the previous section we want to calculate a route for a specific transportation mode. Therefore we have added the `TransportModePanel` UI Kit component. By default it shows all supported transportation modes:
...
First we create a `routingMode` variable and accept a maximum of 5 different route results. The `TransportModePanel` can be customized by setting the desired modes to the `transportModes` array. In the following code snippet we change the default order to start with `bike` first.
```swift
transportModePanel.transportModes = [.bike, .pedestrian, .truck, .car]
transportModePanel.transportMode = .car
```

By setting `transportModePanel.transportMode` you can define which button should be highlighted by default. In this case we choose `car` to be used by default for route calculation.

By setting a delegate function we can get notified whenever a user tapped _and_ changed a transport mode:
```swift
transportModePanel.onModeChanged = onTransportModeChanged
```

Now we need to implement the callback:
```swift
...
As you can see, the callback provides the changed `NMATransportMode` that we can set to our `routingMode`, which will then be used to calculate a new route.
...
In the previous screen the user was able to calculate a route based on his or her waypoint selection and a suitable route mode. Now we want to show a summary for the found routes and their maneuvers on a new screen. As described above we will show this in the `ManeuverViewController` of our UI Kit Primer example app. The `ManeuverViewController` controller holds two UI Kit components:
...
For the sake of this example we show both components on one screen. You may look at our demo app for an alternative User Interface approach. Note that the UI Kit does not promote any specific flow how it's component must be arranged - it all depends on your specific needs and taste.
...
By default the `RouteDescriptionList` shows white `RouteDescriptionItems` on a white list background. We want to change that:
```swift
Styles.shared.routeDescriptionListBackgroundColor = UIColor(red: 0.7, green: 0.7, blue: 0.7, alpha: 1.0)
Styles.shared.routeDescriptionItemBackgroundColor = UIColor(red: 0.95, green: 0.95, blue: 0.95, alpha: 1.0)
```

In addition we want to get notified once a user selects a route. Therefore we implement the `RouteDescriptionListDelegate`:
```swift
...
Like for all UI Kit's list components, we can get notified once a user selects a specific maneuver by tapping on it. To react on this event, we need to conform to the `ManeuverDescriptionListDelegate` protocol and implement it's callback:
```swift
...
```swift
Styles.shared.maneuverDescriptionItemIconColor = .green
```

Tip: Try to play around with other customizable properties.

## Implementing Guidance screen
To finish our quick overview, we want to use the selected route from the previous step to start guidance along that route. For this we only need one new UI Kit component:
...
The `GuidanceManeuverPanel` is a panel where information about the next maneuvers will appear. As with all UI Kit components, it is already configured, so you only need to pass in the current `GuidanceManeuverData`. Therefore we need to implement the `GuidanceManeuverPanelPresenterDelegate` which requires us to implement two callbacks:
```swift
func guidanceManeuverPanelPresenter(_ presenter: GuidanceManeuverPanelPresenter,
                                    didUpdateData data: GuidanceManeuverData) {
    print("onDataChanged: " + data.description)
...
func guidanceManeuverPanelPresenterDidReachDestination(_ presenter: GuidanceManeuverPanelPresenter) {
    ...
    guidanceManeuverPanel.highlightManeuver(textColor: Styles.shared.guidanceManeuverArrivalTextColor)
}
```

While the first callback simply sets the received `GuidanceManeuverData` to the `GuidanceManeuverPanel` by setting it to the `data` property of the panel, the latter informs us when the user has finally reached the destination. In this case, we choose to highlight the last maneuver. Note that the `Styles` singleton already provides the text color property `guidanceManeuverArrivalTextColor`.
...
Congratulations, by following this UI Kit Primer tutorial you have discovered the basic UI Kit components and how they can work together to build extremely powerful apps. Please take a look at the _API Reference_ to learn more about the various UI Kit components. The `API Reference` is available [here](../API_Reference/index.html) - bundled as part of the downloaded UI Kit package.

There you can also find more example code, and our demo application that shows most of the available UI Kit components and capabilities.
