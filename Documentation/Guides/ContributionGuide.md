...
Prerequisites, as of September, 2018:

- Latest [Xcode](https://developer.apple.com/xcode/) (10), which requires macOS High Sierra
...
The iOS version used for testing is `12.0` (latest), and the Simulator used is `iPhone 8`. To run the tests using a different simulator or iOS version, specify the environment variables `DEFAULT_SIMULATOR_NAME` and `DEFAULT_IOS_VERSION`. For instance:
