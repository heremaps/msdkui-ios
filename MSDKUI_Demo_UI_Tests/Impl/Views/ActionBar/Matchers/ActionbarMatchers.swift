...
enum ActionbarMatchers {
    ...
    static func checkWaypointName(withId matcher: GREYMatcher, expectedName: String) {
        EarlGrey.selectElement(with: matcher).perform(
            GREYActionBlock.action(withName: "checkWaypointName") { element, errorOrNil in
                ...
                let item = ActionbarActions.getWaypointItem(inside: cell)
    ...
    static func checkWaypointToLabelDisplayed(forWaypoint matcher: GREYMatcher) {
        ActionbarMatchers.checkWaypointLabel(forWaypoint: matcher, labelToTest: TestStrings.to, shouldBeDisplayed: true)
    }
    ...
    static func checkWaypointFromLabelDisplayed(forWaypoint matcher: GREYMatcher) {
        ActionbarMatchers.checkWaypointLabel(forWaypoint: matcher, labelToTest: TestStrings.from, shouldBeDisplayed: true)
    }
    ...
    static func checkWaypointToLabelHidden(forWaypoint matcher: GREYMatcher) {
        ActionbarMatchers.checkWaypointLabel(forWaypoint: matcher, labelToTest: TestStrings.to, shouldBeDisplayed: false)
    }
    ...
    static func checkWaypointFromLabelHidden(forWaypoint matcher: GREYMatcher) {
        ActionbarMatchers.checkWaypointLabel(forWaypoint: matcher, labelToTest: TestStrings.from, shouldBeDisplayed: false)
    }
    ...
    static func checkWaypointLabel(forWaypoint matcher: GREYMatcher, labelToTest: String, shouldBeDisplayed: Bool) {
        EarlGrey.selectElement(with: matcher).perform(
            GREYActionBlock.action(withName: "checkWaypointLabel") { element, errorOrNil in
                guard
                    errorOrNil != nil,
                    let cell = element as? UITableViewCell else {
                        return false
                }
                ...
                let isDisplayed = item.label.text!.starts(with: labelToTest)
