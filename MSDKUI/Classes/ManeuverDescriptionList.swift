...
@IBDesignable open class ManeuverDescriptionList: UITableView {
    ...
    private func updateStyle() {
        alwaysBounceVertical = Styles.shared.maneuverDescriptionListAlwaysBounceVertical
        separatorInset = Styles.shared.maneuverDescriptionListSeparatorInset
        backgroundColor = Styles.shared.maneuverDescriptionListBackgroundColor
        separatorColor = Styles.shared.maneuverDescriptionListSeparatorColor

        if Styles.shared.maneuverDescriptionListNoEmptyRows == true {
...
extension ManeuverDescriptionList: UITableViewDataSource {
    public func numberOfSections(in _: UITableView) -> Int {
        ...
        if sections == 1 {
            backgroundView = nil
            separatorStyle = Styles.shared.maneuverDescriptionListSeparatorStyle
        } else {
...
extension ManeuverDescriptionList: UITableViewDelegate {
    public func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        // Has any delegate?
        listDelegate?.maneuverSelected(self, index: indexPath.row, maneuver: maneuvers[indexPath.row])
    }
}
