...
/// The protocol used to notify the delegate about changes of the current street name.
public protocol GuidanceCurrentStreetNameMonitorDelegate: AnyObject {
    ...
    func guidanceCurrentManeuverMonitor(_ monitor: GuidanceCurrentStreetNameMonitor, didUpdateCurrentStreetName currentStreetName: String?)
}
...
open class GuidanceCurrentStreetNameMonitor: NSObject {
    ...
    open weak var delegate: GuidanceCurrentStreetNameMonitorDelegate?
...
extension GuidanceCurrentStreetNameMonitor: NMANavigationManagerDelegate {
    public func navigationManager(_ navigationManager: NMANavigationManager, didUpdateManeuvers currentManeuver: NMAManeuver?, _ nextManeuver: NMAManeuver?) {
        delegate?.guidanceCurrentManeuverMonitor(self, didUpdateCurrentStreetName: currentManeuver?.getCurrentStreet())
    }
}
