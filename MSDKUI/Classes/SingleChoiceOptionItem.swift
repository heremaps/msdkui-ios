...
/// This protocol provides the methods to customize the pickerview.
...
@IBDesignable open class SingleChoiceOptionItem: OptionItem {
    ...
    func makeOptions() {
        // Load the nib file
        let nibFile = UINib(nibName: String(describing: SingleChoiceOptionItem.self), bundle: .MSDKUI)
...
extension SingleChoiceOptionItem: UIPickerViewDelegate {
    public func pickerView(_ pickerView: UIPickerView, rowHeightForComponent _: Int) -> CGFloat {
        // If there is a delegate having the optional rowHeight(_:) method implemented, call it
        if let rowHeight = delegate?.rowHeight {
    ...
    public func pickerView(_ pickerView: UIPickerView, widthForComponent _: Int) -> CGFloat {
        // If there is a delegate having the optional rowWidth(_:) method implemented, call it
        if let rowWidth = delegate?.rowWidth {
    ...
    public func pickerView(_ pickerView: UIPickerView, viewForRow row: Int, forComponent _: Int, reusing view: UIView?) -> UIView {
        var pickerLabel = view as! UILabel?

        // If there is a delegate, ask the custom label and otherwise, use the default one
        if let delegate = self.delegate {
            pickerLabel = delegate.makeLabel(pickerView, text: labels[row])
        } else {
            pickerLabel = UILabel()
