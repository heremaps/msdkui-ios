...
@IBDesignable open class WaypointList: UITableView {
    ...
    /// The item color when selected.
    public var itemFlashColor = UIColor.colorBackgroundDark {
        didSet { reloadData() }
    }

    /// The duration, in seconds, the item stays selected before running its action.
    ...
    /// The item 'remove' button's background color.
    public var itemButtonBackgroundColor: UIColor? {
        didSet { reloadData() }
    }

    /// The item buttons' - remove and reorder - tint color.
...
extension WaypointList: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.reorderImageViewTintColor = itemButtonsTintColor

        // Create the content view subview
        let view = WaypointItem()
        view.backgroundColor = itemBackgroundColor
        view.label.textColor = itemTextColor
