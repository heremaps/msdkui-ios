...
@IBDesignable open class RouteDescriptionItem: UIView {
    ...
    func updateStyle() {
        view.backgroundColor = Styles.shared.routeDescriptionItemBackgroundColor
        transportModeImage.tintColor = Styles.shared.routeDescriptionItemTransportModeColor
        barView.progressTintColor = Styles.shared.routeDescriptionItemBarProgressColor
        barView.trackTintColor = Styles.shared.routeDescriptionItemBarTrackColor
        durationLabel.textColor = Styles.shared.routeDescriptionItemDurationTextColor
        delayLabel.textColor = handler != nil && handler.hasDelay ?
            Styles.shared.routeDescriptionItemDelayTextColor : Styles.shared.routeDescriptionItemNoDelayTextColor
        lengthLabel.textColor = Styles.shared.routeDescriptionItemLengthTextColor
        timeLabel.textColor = Styles.shared.routeDescriptionItemTimeTextColor

        // When a UIImage object's tintColor is set it is dimmed a bit, so we create the warning
        // icon with the .alwaysTemplate rendering mode and then get a copy of it tinted with the
        // specified color
        let image = UIImage(named: "RouteDescriptionItem.warning", in: .MSDKUI, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
        warningIcon.image = image?.tint(with: Styles.shared.routeDescriptionItemWarningIconColor)
    }
