...
@IBDesignable open class ManeuverDescriptionItem: UIView {
    ...
    public func setManeuver(maneuvers: [NMAManeuver], position: Int) {
        ...
        if let name = imageName {
            ...
            iconImageView.image = UIImage(named: name, in: .MSDKUI, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
        } else {
    ...
    func refresh() {
        iconImageView.isHidden = !isSectionVisible(Section.icon)
