...
@IBDesignable open class TransportModePanel: UIView {
    ...
    private var selector: UIImage? {
        if selectorImage == nil {
            guard let selectorView = createSelectorView() else {
                return nil
            }

            selectorImage = UIImage(view: selectorView)
        }
    ...
    func createSelectorView() -> UIView? {
        ...
        let selectorView = nibFile.instantiate(withOwner: nil, options: nil)[1] as? UIView

        // Apply the style
        selectorView?.viewWithTag(1000)?.backgroundColor = self.selectorColor
